...
class Board:
    ...
    def check_win(self, row:int, col:int) -> bool|None:
        ...
        for i in range(len(self.__cells[row]) - NUM_CELLS_FOR_WIN + 1):
            states = [cell.get_state() for cell in self.__cells[row][i:i+NUM_CELLS_FOR_WIN] if cell.get_state() is not None]
        ...
        for i in range(len(self.__cells) - NUM_CELLS_FOR_WIN + 1):
            states = [row[col].get_state() for row in self.__cells[i:i+NUM_CELLS_FOR_WIN] if row[col].get_state() is not None]
            if len(states) == NUM_CELLS_FOR_WIN and (sum(states) == NUM_CELLS_FOR_WIN or sum(states) == 0):
                return True

        # Диагональ вправо
        diagonal = self.get_diagonal(self.__cells, row, col, -1, 1) # вниз
        diagonal.extend(self.get_diagonal(self.__cells, row, col, 1, -1)) # вверх
        ...
        # Диагональ влево
        diagonal = self.get_diagonal(self.__cells, row, col, -1, -1) # вверх
        diagonal.extend(self.get_diagonal(self.__cells, row, col, 1, 1)) # вниз
